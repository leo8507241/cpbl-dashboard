...
from common import (data_source_caption, load_intra_game_checkpoints, load_inning_thresholds,
                     load_inning_removal_curve, load_inning_score_quartiles, load_pitch_type_detail,
                     removal_prob_for, zone_for_prob)
import intra_game_fatigue as igf

st.set_page_config(page_title="單場即時換投監控", page_icon="🔥", layout="wide")
st.title("🔥 單場即時換投監控")
data_source_caption("此頁面是「這場比賽此刻」的即時分數，跟其他頁面的賽季規劃疲勞分數是不同單位、不同用途。")
st.caption(
...
checkpoints = load_intra_game_checkpoints()
thresholds = load_inning_thresholds()
removal_curve = load_inning_removal_curve()
score_quartiles = load_inning_score_quartiles()
pt_detail = load_pitch_type_detail()
...
game_df["removal_prob"] = game_df.apply(
    lambda r: removal_prob_for(removal_curve, r["inning"], r["score_with_overlap"]), axis=1)
game_df["zone"] = game_df["removal_prob"].map(zone_for_prob)
game_df["sample_weight"] = game_df["n_pa"].map(igf.sample_weight_multiplier)
...
WEIGHTS = igf.WEIGHTS_WITH_OVERLAP
...
st.caption(
    ...
    f"公式是 `min(累計打席數 / {igf.SAMPLE_RAMP_FULL_AT}, 1.0)`——打席數0時權重乘數0（完全不計入這幾個指標），"
    f"累積到{igf.SAMPLE_RAMP_FULL_AT}個打席（大約投完前兩局）權重乘數封頂1.0（全額計入），中間線性內插。"
